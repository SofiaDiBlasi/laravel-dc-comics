    ...
    <div class="row g-4 py-4">
        <div class="col">
            <form action="{{ route('comics.store') }}" method="post" class="needs-validation">
                ...
                <label>price</label>
                <textarea name="price" class="form-control @error('price') is-invalid @enderror" cols="30" rows="5">{{ old("price") }}</textarea>
                @error("price")
                    <div class="invalid-feedback">{{$message}}</div>
                @enderror

                <label>series</label>
                <textarea name="series" class="form-control @error('series') is-invalid @enderror" cols="30" rows="5">{{ old("series") }}</textarea>
                @error("series")
                    <div class="invalid-feedback">{{$message}}</div>
                @enderror
